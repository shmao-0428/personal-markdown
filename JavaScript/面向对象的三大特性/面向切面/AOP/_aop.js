...
function replaceMethod(target, methodName, aspect, advice) {
  ...
  target[methodName] = (...args) => {
    ...
    if (['before', 'around'].includes(advice)) {
      result = aspect.apply(target, args);
    }

    if (!result && ['before'].includes(advice)) return;

    const returnedValue = originalCode.apply(target, args);
    console.log('%c returnedValue:', 'color:red;font-weight:700;', returnedValue);
...
function inject(target, aspect, advice, pointcut, method = null) {
  console.log('inject arguments>>>', [...arguments]);
  if (pointcut == 'method') {
    if (method != null) {
